...
export function Topbar({ user, status, onMenu }: { user: ShellUser; status: { ai: boolean; tts: boolean; stripe: boolean }; onMenu: () => void }) {
  ...
  const missing = [!status.ai && "AI scripts", !status.tts && "Voiceovers", !status.stripe && "Billing"].filter(Boolean) as string[];
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <span className="hidden items-center gap-1.5 rounded-full border border-amber-500/30 bg-amber-500/10 px-2.5 py-1 text-[11px] font-medium text-amber-300 md:inline-flex">
                <AlertTriangle className="h-3 w-3" /> {missing.length} integration{missing.length > 1 ? "s" : ""} not configured
              </span>
            </TooltipTrigger>
            <TooltipContent>Missing keys for: {missing.join(", ")}. See .env.example.</TooltipContent>
          </Tooltip>
        )}
        <Button asChild size="sm" variant="gradient" className="hidden md:inline-flex">
          <Link href="/scripts"><Plus /> New video</Link>
        </Button>
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-56">
            <DropdownMenuLabel className="normal-case tracking-normal">
              <p className="text-sm font-medium text-foreground">{user.name ?? "Creator"}</p>
              <p className="truncate text-xs font-normal text-muted-foreground">{user.email}</p>
            </DropdownMenuLabel>
            <DropdownMenuSeparator />
            <DropdownMenuItem asChild><Link href="/billing"><CreditCard /> Billing · {user.credits} credits</Link></DropdownMenuItem>
            <DropdownMenuItem asChild><Link href="/settings"><Settings /> Settings</Link></DropdownMenuItem>
            <DropdownMenuSeparator />
            <DropdownMenuItem onSelect={() => signOutAction()} className="text-red-300 focus:text-red-200"><LogOut /> Sign out</DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      </div>
    </header>
