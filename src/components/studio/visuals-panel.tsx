...
export function VisualsPanel({ layers, background, scenes, selectedScene, onLayersChange, onBackgroundChange }: Props) {
  ...
  async function upload(files: FileList | File[]) {
    ...
    for (const file of Array.from(files)) {
      ...
      if (!res.ok) { toast.error(`${file.name}: ${(await res.json()).error ?? "upload failed"}`); continue; }
  ...
  function addLayer(asset: Asset) {
    ...
    if (!scene) return toast.error("Generate a script first so scenes exist.");
    ...
    toast.success(`B-roll added to ${sceneIdx === 0 ? "hook" : sceneIdx === scenes.length - 1 ? "CTA" : `scene ${sceneIdx}`}`);
  }
    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="mt-2 grid grid-cols-3 gap-2">
          {(["gradient", "solid", "grain"] as const).map((t) => (
            <button key={t} onClick={() => onBackgroundChange({ ...background, type: t })} className={cn("rounded-lg border px-2 py-1.5 text-xs capitalize transition", background.type === t ? "border-primary/60 bg-primary/10" : "border-white/10")}>{t}</button>
          ))}
        </div>
        <div className="mt-3 flex items-center gap-2">
          ...
          {background.colors.length < 3 && <Button size="sm" variant="ghost" onClick={() => onBackgroundChange({ ...background, colors: [...background.colors, "#DB2777"] })}><Palette /> Add</Button>}
          {background.colors.length > 1 && <Button size="sm" variant="ghost" onClick={() => onBackgroundChange({ ...background, colors: background.colors.slice(0, -1) })}>Remove</Button>}
        </div>
        <div className="mt-3 grid grid-cols-2 gap-2">
          <label className="flex items-center justify-between rounded-lg border border-white/[0.06] px-3 py-2 text-sm"><span>Vignette</span><Switch checked={background.vignette} onCheckedChange={(v) => onBackgroundChange({ ...background, vignette: v })} /></label>
          <label className="flex items-center justify-between rounded-lg border border-white/[0.06] px-3 py-2 text-sm"><span>Film grain</span><Switch checked={background.grain} onCheckedChange={(v) => onBackgroundChange({ ...background, grain: v })} /></label>
        </div>
      </div>

      <div>
        <div className="flex items-center justify-between"><Label>B-roll for {selectedScene === null ? "selected scene" : selectedScene === 0 ? "hook" : selectedScene === scenes.length - 1 ? "CTA" : `scene ${selectedScene}`}</Label><span className="text-[11px] text-muted-foreground">Click a scene on the timeline</span></div>
        <div
        ...
        >
          ...
          <p className="mt-2 text-sm font-medium">Drop images or clips</p>
          <p className="text-xs text-muted-foreground">PNG, JPG, WebP, MP4 · up to 50 MB</p>
        </div>
        {assets.length > 0 && (
      ...
      <div>
        <Label className="inline-flex items-center gap-1"><Layers className="h-3 w-3" /> Layers ({layers.length})</Label>
        {layers.length === 0 ? (
          <p className="mt-2 text-xs text-muted-foreground">No visual layers. The animated background plays behind captions.</p>
        ) : (
            ...
            {[...layers].sort((a, b) => a.startMs - b.startMs).map((l) => (
              <li key={l.id} className="flex items-center gap-3 rounded-lg border border-white/[0.06] bg-white/[0.02] p-2">
                ...
                <div className="min-w-0 flex-1 space-y-1">
                  <p className="truncate text-xs font-medium">{l.sceneIndex === 0 ? "Hook" : l.sceneIndex === scenes.length - 1 ? "CTA" : `Scene ${l.sceneIndex}`} · {(l.startMs / 1000).toFixed(1)}–{(l.endMs / 1000).toFixed(1)}s</p>
                  <div className="flex gap-1">
                    <Select value={l.kenBurns} onValueChange={(v) => update(l.id, { kenBurns: v as VisualLayer["kenBurns"] })}>
                      <SelectTrigger className="h-7 text-[11px]"><SelectValue /></SelectTrigger>
                      <SelectContent><SelectItem value="in">Zoom in</SelectItem><SelectItem value="out">Zoom out</SelectItem><SelectItem value="pan-left">Pan left</SelectItem><SelectItem value="pan-right">Pan right</SelectItem><SelectItem value="none">Static</SelectItem></SelectContent>
                    </Select>
