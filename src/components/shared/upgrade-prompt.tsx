...
export function UpgradePrompt({ title = "Vous n'avez plus de crédits", body = "Passez à un forfait supérieur ou achetez un pack de crédits pour continuer à créer.", compact = false }: { title?: string; body?: string; compact?: boolean }) {
  return (
    <div className={`relative overflow-hidden rounded-xl border border-primary/30 bg-primary/10 ${compact ? "p-4" : "p-6"}`}>
      ...
      <div className="relative flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <Button asChild variant="gradient" size={compact ? "sm" : "default"}>
          <Link href="/billing"><Zap /> Voir les forfaits</Link>
        </Button>
      </div>
    </div>
  );
