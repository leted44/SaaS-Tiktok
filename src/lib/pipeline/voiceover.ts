...
/**
 * Voice a script for a user. Shared by the studio's button and the autopilot,
 * so plan limits on premium and cloned voices apply the same way to both.
 */
export async function createVoiceover(user: User, data: z.output<typeof voiceoverRequestSchema>): Promise<VoiceoverSummary> {
