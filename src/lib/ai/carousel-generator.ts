...
export const carouselFields = z.object({
  ...
  ctaTitle: z.string().describe("The closing headline: one concrete takeaway or first action the reader can do today. 80 characters maximum."),
  ctaBody: z.string().describe("One or two short sentences that make the takeaway stick. 180 characters maximum. Do not ask for a save or a follow here — the design adds that."),
});
...
export async function generateCarousel(input: CarouselInput): Promise<CarouselSlide[]> {
  ...
  return [
    { id: nanoid(8), kind: "cover", kicker: fit(out.coverKicker, L.cover.kicker), title: fit(out.coverTitle, L.cover.title), body: fit(out.coverSubtitle, L.cover.body) },
    ...out.slides.slice(0, 8).map((s) => ({ id: nanoid(8), kind: "content" as const, kicker: "", title: fit(s.title, L.content.title), body: fit(s.body, L.content.body) })),
    { id: nanoid(8), kind: "cta", kicker: fit(out.ctaKicker, L.cta.kicker), title: fit(out.ctaTitle, L.cta.title), body: fit(out.ctaBody, L.cta.body) },
  ];
}
